...
-- Phase 1B (онлайн-оплата): заказ-по-кнопке в боте пишет БОТ (owner, грантов не требует);
-- заказ-«счёт из диалога» создаёт ПАНЕЛЬ (insert), и она же из вебхука отмечает оплату
...
-- ── База знаний RF-RAG (kb_documents/kb_chunks) — объекты в db/schema_kb.sql, ПОСЛЕ него ──
-- Панель/ингест пишут документы и чанки (+эмбеддинги); БОТ (owner) только ЧИТАЕТ kb_chunks
-- при retrieval — грант ему не нужен. kb_chunks.id = identity → отдельный грант на sequence
-- НЕ требуется (в отличие от bigserial выше). Зеркало грантов из schema_kb.sql.
...
-- выше снимает всё, здесь восстанавливаем). Матрица «кто что пишет»:
--   tenants            SELECT, INSERT, UPDATE — платформа-админ заводит/гасит тенанта (без delete)
--   memberships        CRUD — доступ операторов к тенанту (отзыв = delete)
--   tenant_settings    SELECT, INSERT, UPDATE — панель пишет, бот (owner) читает
--   plans              SELECT, INSERT, UPDATE — тарифы правит только платформа-админ (UI-гейт)
--   subscriptions      SELECT, INSERT, UPDATE — выставление/продление; автосписания ведёт бот-воркер
--   payments           SELECT, INSERT, UPDATE — создание платежа + отметка из вебхука
--   webhook_events     SELECT, INSERT, UPDATE — дедуп повторных доставок ЮKassa
--   credit_wallets     SELECT, INSERT, UPDATE — пополнение из вебхука; списание — бот (owner)
...
grant select, insert, update         on plans                 to panel_rw;
...
grant select, insert, update         on credit_wallets        to panel_rw;
