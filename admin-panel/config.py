...
# Каталог тарифов. price — в рублях; payable=False (Индивидуальный) не оплачивается
# онлайн и ведёт на заявку (SERVICE_CONTACT_URL). price_display/subtitle/features —
# витринные строки карточки.
#
# ⚠️ Ключи quota/overage — ЛЕГАСИ снятой модели «сообщения ИИ + доплата сверх квоты».
# УДАЛЯТЬ ИХ НЕЛЬЗЯ: plan["quota"] читается по индексу при создании счёта (app.py, поток
# /subscription/select) и в scripts/cut_over_metering.py — KeyError уронит всю оплату.
# Из ВИДИМЫХ клиенту строк снятая модель убрана: движок тарифицирует токены
# (shared/metering.py валит RuntimeError на billing_mode='per_message'), а оферта п. 4.1
# прямо говорит, что обязательства определяются включённым лимитом и Курсом тарификации,
# а не количеством ответов. Источник истины по курсу — таблица billing_token_rate;
# строка ниже дублирует значение, опубликованное в оферте п. 1.7 (1,5 ₽ за 1000 токенов).
SERVICE_PLANS = {
    "econom": {
        "name": "Эконом", "payable": True, "price": 3750, "quota": 500, "overage": 7.5,
        "price_display": "3 750 ₽ за период",
        "subtitle": "Для небольших компаний",
        "features": ["Включённый лимит использования 3 750 ₽",
                     "Курс тарификации 1,5 ₽ за 1000 токенов",
                     "Неиспользованный остаток на следующий период не переносится",
                     "Без ограничений по количеству каналов"],
    },
    "start": {
        "name": "Стартовый", "payable": True, "price": 7500, "quota": 1500, "overage": 5,
        "price_display": "7 500 ₽ за период",
        "subtitle": "Для тех, кто хочет автоматизировать общение с клиентами",
        "features": ["Включённый лимит использования 7 500 ₽",
                     "Курс тарификации 1,5 ₽ за 1000 токенов",
                     "Неиспользованный остаток на следующий период не переносится"],
    },
    "custom": {
        ...
        "features": ["Расширенный включённый лимит по договорённости",
                     "Приоритетная поддержка", "Неограниченное дообучение ИИ",
                     "Личный менеджер"],
        "cta": "Оставить заявку",
    },
}
