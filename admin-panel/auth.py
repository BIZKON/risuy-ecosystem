...
# Текст отказа партнёрского гейта. Вынесен константой, чтобы страница ошибки могла
# показать партнёру ЕГО ссылки: сессии у неё нет, а сравнивать строку в двух местах —
# верный способ разъехаться.
PARTNER_DENIED = "Раздел недоступен"


def landing_for(role: str, next_path: str) -> str:
    """Куда вести человека сразу после входа.

    🔴 Партнёру общий дашборд закрыт, а next по умолчанию — «/». Без этой ветки партнёр
    вводит пароль и получает 403 на первом же экране; ровно так и случилось у живого
    партнёра 17.08. Запрошенный next уважаем, только если он ведёт в его же раздел.
    """
    if role == "partner" and not partner_may_access(next_path):
