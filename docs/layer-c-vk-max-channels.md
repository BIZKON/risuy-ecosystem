...
Статус: **✅ РЕАЛИЗОВАНО (C0–C3).** Каналы VK/MAX в проде (миграции `migrate_layer_c_identity`,
`migrate_c3_channel_outbound`; код в `bot-telegram/` + `admin-panel/`). Этот документ — исходный
дизайн-черновик, сохранён как ИСТОРИЯ решений; фактические места — в коде, не здесь. Основано на
исследовании 4 агентов (карта архитектуры + VK Bot API + MAX Bot API + модель данных).
...
> ✅ Все фазы C0–C3 РЕАЛИЗОВАНЫ и в проде. Ниже — исходный план (история).
...
5. ✅ **Панель «Диалоги»** (C3) — СДЕЛАНО: гейт `_chat.html:24` теперь `lead.can_reply` (канально),
   `enqueue_manual_reply`/`outbox`/`worker._drain_outbox_channels` пишут по `messenger`, плейсхолдеры
   канал-агностичны. См. коммит C3 + `migrate_c3_channel_outbound`.
