...
## Известные ограничения v1 / отложено (по итогам адверсариального ревью)
- **Резолвер слоёв подключён только в TG (`t_text`).** VK/MAX (`_vk_respond`/`_max_respond`) и `t_document`
  остаются на легаси `get_tenant_ai_overrides` — каналы VK/MAX не подняты боевыми токенами. Проводка туда —
  следующий инкремент (заменить вызов на `resolve_team_agent_cfg(source='vk'/'max', ...)`).
- **`is_orchestrator` — без UI** (флаг в БД/CRUD есть, чекбокса в панели нет): оркестрация = СП-4.
- **Легаси `/my-agent`** (редактор одной персоны) остаётся доступен прямым URL (из меню убран). После бэкфилла
  резолвер предпочитает `team_agents`, поэтому правки через `/my-agent` на ответы не влияют → follow-up: редирект
  `/my-agent`→`/my-team` либо баннер (LOW, не блокер).
- **Per-agent `escalation_chat_id`** — ПОДКЛЮЧЁН (override в `escalate()` из `cfg`, фолбэк на общий адрес тенанта).
- **Привязка «по каналу»** — форма в UI ДОБАВЛЕНА (`/my-team/channel`), slug валидируется против enabled-агентов.
- **Панель-созданный cloud_ai-агент без `agent_id`** — наследует cloud-ai агента тенанта (легаси) в резолвере,
  иначе молчал бы. Свой `agent_id`/`backend` на агента в UI пока не задаётся (провижининг owner-side).
