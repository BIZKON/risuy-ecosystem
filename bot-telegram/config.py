...
# ── Суточный потолок публичного демо-чата на сайте (дыра C) ───────────────────
# Веб-виджет info.pro-agent-ai.ru ходит в платный шлюз без учёта (метеринг на этом пути
# не списывается), а минутные rate-limit'ы живут в ПАМЯТИ процесса и обнуляются каждым
# редеплоем. Потолок — суточный и в БД, в двух измерениях:
#   • DEMO_CHAT_DAILY_LIMIT  — сколько ОТВЕТОВ в сутки отдаём посетителям сайта;
#   • DEMO_CHAT_DAILY_TOKENS — сколько ТОКЕНОВ в сутки суммарно уйдёт в шлюз. Без него
#     потолок в штуках не является потолком в рублях: объём одного запроса задаёт сам
#     отправитель (история до 24 сообщений × 2000 символов → разброс цены слота ×10-40).
#     На боевых числах связывающим обычно будет бюджет токенов: 100 тяжёлых запросов дали бы
#     около 2 млн токенов, то есть миллион выбирается примерно на 40-м таком запросе.
#
# ⚠️ Семантика нуля у двух констант РАЗНАЯ (перепутать = снять бюджет, думая, что закрыл витрину):
#   • DEMO_CHAT_DAILY_LIMIT = 0 — kill-switch витрины: первый же запрос получает вежливый
#     текст «вернусь завтра», алерт владельцу при этом НЕ шлём (иначе он был бы ежедневным);
#   • DEMO_CHAT_DAILY_TOKENS = 0 — бюджет токенов ВЫКЛЮЧЕН (в bot.py весь блок стоит под
#     `if config.DEMO_CHAT_DAILY_TOKENS > 0`), расход ограничен только числом ответов.
#     Витрину этим не закрыть. Следствие того же: мусор или отрицательное число в env
#     _env_int клампит в 0 (minimum=0) — то есть порча env выключает БЮДЖЕТ, а не витрину.
# Боевые дефолты зашиты в код намеренно: заводить новый env через UI Timeweb рискованно
# (правка env затирает run_cmd). Число ответов дополнительно переопределяется БЕЗ деплоя
# апсертом app_settings['demo_chat_daily_limit'] (см. db.get_demo_chat_cfg) — именно
# апсертом, а не UPDATE: этот ключ никто не создаёт, в коде он только читается, и на чистой
# базе UPDATE тронул бы 0 строк («UPDATE 0»), оставив витрину закрытой:
#   insert into app_settings (key, value) values ('demo_chat_daily_limit','<N>')
#     on conflict (key) do update set value = excluded.value, updated_at = now();
# TODO(владелец, решение Р3(4)): ужесточение _CHAT_MAX_MESSAGES 24→8 / _CHAT_MAX_LEN 2000→1000
# и/или суточный per-IP ключ — тогда цена одного слота перестанет плавать на порядок.
DEMO_CHAT_DAILY_LIMIT = _env_int("DEMO_CHAT_DAILY_LIMIT", 100, minimum=0)
DEMO_CHAT_DAILY_TOKENS = _env_int("DEMO_CHAT_DAILY_TOKENS", 1_000_000, minimum=0)
