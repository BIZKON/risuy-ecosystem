...
def valid_persona(slug, allowed) -> str:
    ...
    ⚠️ Допущение: пресеты — РОЛИ (liya/mark/…), не персоналии. Если появится именной пресет
    (имя конкретного сотрудника) — пересмотреть, должен ли ai_persona попадать в обезличенный набор."""
